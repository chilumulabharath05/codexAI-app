...
async def get_messages(repo_id: str, session_id: str, db: AsyncSession = Depends(get_db)):
    ...
    return [{"id": m.id, "role": m.role, "content": m.content, "sources": m.sources or [], "created_at": m.created_at.isoformat()} for m in r.scalars()]


@router.post("/{repo_id}/sessions/{session_id}/messages")
async def send_message(repo_id: str, session_id: str, payload: Dict, db: AsyncSession = Depends(get_db)):
    ...
    ai_msg = ChatMessage(session_id=session_id, role="assistant",content=result["answer"], sources=result["sources"])
    db.add(ai_msg)
    await db.commit()
